-- 0030 · Intake v3 · the client picks a down payment and a payment duration, never a monthly amount
--
-- Report v3 wins over spec v2 where they differ (owner decision, docs/gap-rapport-v3.md L-2, L-3, L-9, N-9).
-- §6:  the client chooses the down payment and the duration; the system computes the monthly amount, so the
--      public form no longer asks for an installment. Old demands keep their installment snapshot.
-- §7-§8: durations are a Back Office list in months (3/5/7 years seeded, 84 months at most today); the admin
--      adds 24/48/72 without a developer.
-- §14 / N-9: «نحب حل تمويل بنكي» is recorded now to measure demand; bank financing itself stays Phase 3.
-- §40: the first form keeps the desired area and the purchase goal and adds the budget, the payment duration
--      and «هل يحب زيارة». Priority is not in the report and leaves the public form (still accepted if sent).
-- LEAD-01 / LEAD-02: every new choice comes from a Back Office list and is snapshotted on the demand.
-- §17: the closing call to action says «سجّل اهتمامك», not «سجّل مطلبك».

-- ---------------------------------------------------------------------------
-- Lists: payment durations (seeded from §8) and budgets (empty until AgriZed fills it)
-- ---------------------------------------------------------------------------

insert into public.option_lists (key, label_ar, value_kind, description_ar) values
  ('duration', 'مدة الدفع', 'number_range',
   'مدد الدفع التي يختار منها الحريف، بالأشهر في min_number وmax_number (التقرير v3 §8). محرّك العروض وبطاقات /projects يقرؤون أكبر مدة مفعّلة.'),
  ('budget', 'الميزانية', 'money',
   'مجالات الميزانية الجملية للحريف (التقرير v3 §40). تبقى فارغة حتى تعمّرها AgriZed، والاستمارة تخفي السؤال ما دامت القائمة فارغة.')
on conflict (key) do nothing;

-- sort_order equals the months, so durations the admin adds later (24, 48, 72) fall into place.
insert into public.option_items (list_key, code, label_ar, label_fr, min_number, max_number, sort_order) values
  ('duration', 'd_36', '3 سنوات', '3 ans', 36, 36, 36),
  ('duration', 'd_60', '5 سنوات', '5 ans', 60, 60, 60),
  ('duration', 'd_84', '7 سنوات', '7 ans', 84, 84, 84)
on conflict (list_key, code) do nothing;

-- ---------------------------------------------------------------------------
-- Snapshot columns on the demand (LEAD-02)
-- ---------------------------------------------------------------------------

alter table public.interest_requests
  alter column installment_option_id drop not null,
  alter column installment_label_ar drop not null,
  add column if not exists duration_option_id   uuid references public.option_items (id),
  add column if not exists duration_label_ar    text,
  add column if not exists duration_months      integer,
  add column if not exists budget_option_id     uuid references public.option_items (id),
  add column if not exists budget_label_ar      text,
  add column if not exists budget_min_millimes  bigint,
  add column if not exists budget_max_millimes  bigint,
  add column if not exists wants_visit          boolean,
  add column if not exists wants_bank_financing boolean;

create index if not exists interest_requests_duration_idx on public.interest_requests (duration_months);

comment on column public.interest_requests.installment_option_id is
  'Monthly installment the client chose before report v3 §6. New demands leave it empty: the client picks a duration and the system computes the monthly amount.';
comment on column public.interest_requests.installment_label_ar is
  'Label of the installment as displayed when chosen (LEAD-02). Empty on demands made after report v3 §6.';
comment on column public.interest_requests.duration_option_id is
  'Payment duration chosen from the ''duration'' list (report v3 §8). Empty when the client did not answer.';
comment on column public.interest_requests.duration_label_ar is
  'Label of the chosen duration as displayed (LEAD-02).';
comment on column public.interest_requests.duration_months is
  'Months of the chosen duration, copied from the list item (LEAD-02); later edits of the list never change it.';
comment on column public.interest_requests.budget_option_id is
  'Overall budget range chosen from the ''budget'' list (report v3 §40). Optional.';
comment on column public.interest_requests.budget_label_ar is
  'Label of the chosen budget as displayed (LEAD-02).';
comment on column public.interest_requests.budget_min_millimes is
  'Lower bound of the chosen budget in millimes (LEAD-02).';
comment on column public.interest_requests.budget_max_millimes is
  'Upper bound of the chosen budget in millimes; empty for an open-ended range (LEAD-02).';
comment on column public.interest_requests.wants_visit is
  '«تحب تزور الأرض؟» (report v3 §40): true yes, false not yet, empty when not answered.';
comment on column public.interest_requests.wants_bank_financing is
  '«نحب حل تمويل بنكي» (report v3 §14, decision N-9): recorded to measure demand only. Empty when not answered.';

-- ---------------------------------------------------------------------------
-- Intake: duration, budget, visit and bank financing; the installment becomes optional
-- ---------------------------------------------------------------------------
...
begin
  ...
  if nullif(p->>'installment_option_id', '') is not null then
  ...
  if nullif(p->>'duration_option_id', '') is not null then
...
-- ---------------------------------------------------------------------------
-- CRM view and search: the new answers are listed and filtered (report v3 §44)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Demand report: durations, visits and bank financing (report v3 §49)
-- ---------------------------------------------------------------------------
...
comment on function public.crm_demand_stats(date, date, boolean) is
  'Demand report for the dashboard and analytics (spec v2 §46, §47, §55, report v3 §49). Security invoker: figures cover only the files the caller may see. p_people counts distinct persons in every breakdown; trees never count duplicates (MIL-01). by_duration lists active durations, retired ones still carried by demands, then «no answer»; visit_yes and bank_financing_yes count the «yes» answers.';
...
insert into public.settings (key, value, value_type, group_key, label_ar, description_ar, is_public, sort_order) values
  ('start.row_duration', to_jsonb('مدة الدفع'::text), 'text', 'site', 'صف مدة الدفع في الملخّص',
   'اسم خيارات المدة وصفّها في بطاقة الملخّص في صفحة /start (التقرير v3 §8 و§12). مسودة، تُراجع من الإدارة.', true, 163),
  ('start.row_duration_fr', to_jsonb('Durée de paiement'::text), 'text', 'site', 'صف مدة الدفع في الملخّص (فرنسي)',
   'السطر الفرنسي تحت اسم خيارات المدة في صفحة /start. مسودة، تُراجع من الإدارة.', true, 164)
on conflict (key) do nothing;

-- The /start capacity hint named the monthly installment (0019 seed); report v3 §6 asks a duration instead.
update public.settings
set value = to_jsonb('اختر التسبقة ومدة الدفع اللي يناسبوك. هذي قدرتك، موش سعر مشروع.'::text),
    description_ar = concat_ws(' ', description_ar,
      'مسودة، تُراجع من الإدارة: «القسط» عُوّض بـ«مدة الدفع» (تقرير v3 البند 6).')
where key = 'start.capacity_hint'
  and value = to_jsonb('اختر التسبقة والقسط اللي يناسبوك. هذي قدرتك، موش سعر مشروع.'::text);

update public.settings
set value = to_jsonb('Choisissez l''apport et la durée de paiement qui vous conviennent. C''est votre capacité, pas le prix d''un projet.'::text),
    description_ar = concat_ws(' ', description_ar, 'مسودة، تُراجع من الإدارة.')
where key = 'start.capacity_hint_fr'
  and value = to_jsonb('Choisissez l''apport et la mensualité qui vous conviennent. C''est votre capacité, pas le prix d''un projet.'::text);

-- Only while the 0016 seed is untouched, so a title the owner already edited is kept.
update public.settings
set value = to_jsonb('سجّل اهتمامك في مشروع المليون زيتونة'::text),
    description_ar = concat_ws(' ', description_ar,
      'مسودة، تُراجع من الإدارة: «سجّل مطلبك» عُوّضت بـ«سجّل اهتمامك» (تقرير v3 البند 17).')
where key = 'site.final_cta_title'
  and value = to_jsonb('سجّل مطلبك في مشروع المليون زيتونة'::text);
